...
class CitizenAddToQueue(Resource):
    ...
    def post(self, id):
        ...
        citizens = Citizen.query \
                .options(joinedload(Citizen.service_reqs, innerjoin=True).joinedload(ServiceReq.periods, innerjoin=True).options(raiseload(Period.sr),joinedload(Period.csr, innerjoin=True).raiseload('*')),joinedload(Citizen.office).options(raiseload('*')),raiseload(Citizen.counter),raiseload(Citizen.user)) \
